...
<div class="col-md-12">
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">
                <button type="button" class="btn btn-default btn-sm my-2" data-toggle="modal" data-target=".tambah-user">
            ...
            <tbody>
                @foreach ($user as $role => $data)
                  <tr>
                    ...
                    <td>{{ $data->count() }}</td>
                    <td>
                      <a href="{{ route('user.show', Crypt::encrypt($role)) }}" class="btn btn-info btn-sm"><i class="nav-icon fas fa-search-plus"></i></a>
                    </td>
                  </tr>
                @endforeach
            </tbody>
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="form-group">
                  ...
                  <input id="email" type="email" placeholder="{{ __('user@email.com') }}" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ old('email') }}" autocomplete="email">
                  @error('email')
