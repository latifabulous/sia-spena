                ...
                <tr>
                    ...
                    <td>
                        <form action="{{ route('jadwal.kill', $data->id) }}" method="post">
                            @csrf
                            @method('delete')
                            <a href="{{ route('jadwal.restore', Crypt::encrypt($data->id)) }}" class="btn btn-success btn-sm"><i class="nav-icon fas fa-undo"></i> &nbsp; Restore</a>
                            <button class="btn btn-danger btn-sm  ml-2"><i class="nav-icon fas fa-trash-alt"></i> &nbsp; Hapus</button>
                        </form>
                    </td>
                </tr>
                @endforeach
